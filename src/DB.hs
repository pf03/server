...
whereAll q conditions = DB.concat2 DB.where_ q $ DB.all conditions

whereAllM :: MError m => Query -> [m Query] -> m Query
whereAllM q mconditions = do 
    (return . DB.concat2 DB.where_ q . DB.all) <$$> mconditions

any :: [Query] -> Query
any = DB.concat [sql|OR|]

all :: [Query] -> Query
all = DB.concat DB.and

concat :: Query -> [Query] -> Query
concat splitter [] = mempty
concat splitter [x] = x
concat splitter (x:xs) = concat2 splitter x (DB.concat splitter xs)
...
inList field [] = "FALSE"
inList field values = template [sql|{0} IN ({1})|] [field, DB.concat "," $ map q values] 
...
list :: [Query] -> Query
list qs = template [sql|({0})|] [DB.concat "," qs] 

--emp = DB.query_ [sql|SELECT posts.id FROM posts WHERE FALSE|] ::T [Only Int]  --or TRUE
